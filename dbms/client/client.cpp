	...
	std::istringstream &args)
{
    ...
    if (!(args >> value.hashed_passwod && args >> value._name))
    {
	...
	std::istringstream &args)
{
    ...
    if (!(args >> name))
    {
		throw std::runtime_error("Expected struct _name");
    }
	if (name.size() > db_ipc::MSG_STRUCTS_NAME_SIZE)
    {
		throw std::runtime_error("Invalid struct _name");
    }
	...
	size_t request_counter)
{
	...
	switch (msg.status)
	{
		...
		case db_ipc::command_status::TOO_BIG_STRUCT_NAME:
			throw std::runtime_error("Too big struct _name");
	...
	size_t &request_counter)
{
    ...
    if (&stream == &std::cin)
    {
        std::cout << "Enter the commands:\n\n" <<
                  "add - adding a new entry\n" << "update - update a entry\n" << "dispose - deleting an existing entry\n" <<
                  "obtain - reading a entry\n" << "obtain_between - reading a set of entries from a range\n\n" <<
                  "addPool - adding a poll\n" << "disposePool - deleting a poll\n" <<
                  "addSchema - adding a scheme\n" << "disSchema - deleting a scheme\n" <<
                  "addCollection - adding a collection\n" << "disposeCollection - deleting a collection\n\n" <<
                  "executeFile - executing instructions from a file\n" << "reconnect - reconnect to the server\n" <<
                  "stop - finish executing operations\n" << std::endl;
    }
