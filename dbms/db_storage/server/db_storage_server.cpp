    ...
    while (run_flag)
    {
		...
		switch (msg.cmd)
		{
			...
			case db_ipc::command::ADD_POOL:
			{
				...
				{
                    logger->error(log_start + "Failed to add pool due to invalid _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::too_big_struct_name_exception const &)
				{
                    logger->error(log_start + "Failed to add pool due to too big _name");
					msg.status = db_ipc::command_status::TOO_BIG_STRUCT_NAME;
				}
                catch (db_storage::invalid_path_exception const &)
                {
                    logger->error(log_start + "Invalid path");
                    msg.status = db_ipc::command_status::INVALID_PATH;
                }
			...
			case db_ipc::command::ADD_SCHEMA:
			{
				...
				{
                    logger->error(log_start + "Failed to add schema due to invalid _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::too_big_struct_name_exception const &)
				{
                    logger->error(log_start + "Failed to add schema due to too big _name");
					msg.status = db_ipc::command_status::TOO_BIG_STRUCT_NAME;
				}
                catch (db_storage::invalid_path_exception const &)
                {
			...
			case db_ipc::command::ADD_COLLECTION:
			{
				...
				{
                    logger->error(log_start + "Failed to add collection due to invalid _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::too_big_struct_name_exception const &)
				{
                    logger->error(log_start + "Failed to add collection due to too big _name");
					msg.status = db_ipc::command_status::TOO_BIG_STRUCT_NAME;
				}
				catch (db_storage::insertion_of_struct_failure const &)
				{
			...
			case db_ipc::command::ADD:
			{
				...
				{
                    logger->error(log_start + "Failed to insert key '" + msg.login + "' due to invalid struct _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::invalid_path_exception const &)
				{
			...
			case db_ipc::command::UPDATE:
			{
				...
				{
                    logger->error(log_start + "Failed to update key '" + msg.login + "' due to invalid struct _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::invalid_path_exception const &)
				{
			...
			case db_ipc::command::DISPOSE:
			{
				try
				{
					...
					msg.hashed_password = value._count_detailes;
					strcpy(msg.name, value._name.c_str());
				}
				catch (db_storage::setup_failure const &)
				{
				...
				{
                    logger->error(log_start + "Failed to dispose key '" + msg.login + "' due to invalid struct _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::invalid_path_exception const &)
				{
			...
			case db_ipc::command::OBTAIN:
			{
				...
				{
                    logger->error(log_start + "Failed to obtain key '" + msg.login + "' due to invalid struct _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::invalid_path_exception const &)
				{
			...
			case db_ipc::command::OBTAIN_BETWEEN:
			{
				...
				{
                    logger->error(log_start + "Failed to obtain between keys " + keys + " due to invalid struct _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::invalid_path_exception const &)
				{
				...
				for (int i = 0; i < range.size(); ++i)
                {
                    ...
                    msg.hashed_password = range[i].second._count_detailes;
                    strcpy(msg.name, range[i].second._name.c_str());
			...
			case db_ipc::command::OBTAIN_MIN:
            {
                try
                {
					...
					msg.hashed_password = kvp.second._count_detailes;
					strcpy(msg.name, kvp.second._name.c_str());
                }
				catch (db_storage::setup_failure const &)
				{
				...
				{
                    logger->error(log_start + "Failed to obtain min key due to invalid struct _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::invalid_path_exception const &)
				{
			...
			case db_ipc::command::OBTAIN_MAX:
            {
				...
				{
                    logger->error(log_start + "Failed to obtain max key due to invalid struct _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::invalid_path_exception const &)
				{
			...
			case db_ipc::command::OBTAIN_NEXT:
            {
				...
				{
                    logger->error(log_start + "Failed to obtain next of key '" + msg.login + "' due to invalid struct _name");
					msg.status = db_ipc::command_status::INVALID_STRUCT_NAME;
				}
				catch (db_storage::invalid_path_exception const &)
				{
